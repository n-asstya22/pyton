...
while is_game == "y":
    ...
    while get_card == "y":
        ...
        if sum(hand_player) > 21 and 11 in hand_player:  #если сумма больше 21 и туз в руке
        ...
        print(f"Первая рука компьютера: {hand_computer[0]}")

        if score_player > 21:
            get_card = "n"
        else:
            get_card = input("Берем карту? y - др, n - нет")
    while sum(hand_computer) < 17:
            hand_computer.append(random.choice(card))

            if sum(hand_computer) > 21 and 11 in hand_computer:  # если сумма больше 21 и туз в руке
